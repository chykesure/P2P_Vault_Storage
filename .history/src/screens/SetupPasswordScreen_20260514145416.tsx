 ...
 *
 * FIX: Changed setup → setupEncryption (matches EncryptionContext).
 * FIX: Added local state for isLoading/error.
 * FIX: Added navigation.reset() after successful setup.
 */
...
export function SetupPasswordScreen() {
  ...
  const handleSetup = async (password: string) => {
    try {
      ...
      if (success) {
        // Force navigation to Home after successful setup
