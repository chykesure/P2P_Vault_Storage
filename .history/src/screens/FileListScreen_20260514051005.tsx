...
export function FileListScreen({ route }: any) {
  const { file: fileFromParams } = route.params;
  const { downloadFile, deleteFile } = useFileVault();
  const [isDownloading, setIsDownloading] = useState(false);

  // Use file passed via navigation params (instant, no loading)
  const file = fileFromParams as VaultFile | null;
