...
export function FileListScreen() {
  ...
  // Auto-refresh files every time this screen gains focus
  useFocusEffect(
    useCallback(() => {
      refreshFiles();
    }, [refreshFiles]),
  ...
  // Derive filtered list from files + search query
  const derivedFilteredFiles = (() => {
  ...
  const handleDelete = useCallback(
    (cid: string) => {
      Alert.alert(
        'Delete File',
        'Remove this file record from the blockchain? The encrypted file may remain on IPFS until garbage collected.',
        [
    ...
    <View style={styles.container}>
      ...
      <View style={styles.resultsRow}>
        <Text style={styles.resultsText}>
          {derivedFilteredFiles.length} {derivedFilteredFiles.length === 1 ? 'file' : 'files'}
        </Text>
      </View>

      {/* File List */}
      <FlatList
        data={derivedFilteredFiles}
        keyExtractor={item => item.cid}
        renderItem={({ item }) => (
          <FileCard
            file={item}
            onPress={() => navigation.navigate('FileDetail', { cid: item.cid })}
        ...
        contentContainerStyle={derivedFilteredFiles.length === 0 ? styles.emptyListContent : undefined}
