...
// Contract stats replaced with IPFS-based file index (gas-free)
...
export function HomeScreen() {
  ...
  const { isConnected, address, openModal, projectId, error } = useWeb3();
  ...
  const isProjectIdConfigured = projectId && projectId !== 'YOUR_PROJECT_ID' && projectId.length > 10;
  ...
  // Show error alert if project ID is not configured
  useEffect(() => {
    if (error) {
          ...
          <View style={styles.notConnectedContainer}>
            ...
            {!isProjectIdConfigured && (
              <View style={styles.warningBox}>
                <Icon name="warning-outline" size={20} color="#F39C12" />
                <Text style={styles.warningText}>
                  WalletConnect Project ID not configured. Get a FREE one from cloud.walletconnect.com and set it in app.json
                </Text>
              </View>
            )}
              ...
              {files.length === 0 ? (
                ...
                files.slice(0, 5).map(file => (
                  <FileCard
                    ...
                    file={file}
                    onPress={() =>
                      navigation.navigate('FileDetail', { file, cid: file.cid })
                    }
                  />
                ))
              )}
