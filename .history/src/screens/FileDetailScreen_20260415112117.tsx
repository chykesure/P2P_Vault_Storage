...
export function FileDetailScreen({ route }: any) {
  // Get the full file object from navigation params (passed from FileListScreen)
  const initialFile = route.params?.file as VaultFile | undefined;
  const { downloadFile, deleteFile } = useFileVault();
  const [file, setFile] = useState<VaultFile | null>(initialFile || null);
