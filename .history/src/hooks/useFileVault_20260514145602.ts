 ...
 *
 * FIX: Added await to encrypt() call.
 * FIX: Convert Uint8Array → base64 string before encrypt (encrypt expects string).
 * FIX: Changed encryptionResult.encryptedData → encryptionResult.encrypted.
 * FIX: Added getFileByCid method for FileDetailScreen.
 * FIX: Added actual decryption in downloadFile.
 * FIX: Saved encryptedKey/iv to file index during upload.
 */
...
export function useFileVault(): UseFileVaultReturn {
  ...
  const uploadFile = useCallback(
    async (
      ...
      try {
        // ★ FIX Step 1: Convert Uint8Array → base64 string (encrypt expects a string)
        ...
        // ★ FIX Step 2: Encrypt the base64 string (with await!)
        ...
        // ★ FIX Step 3: Upload encrypted data to IPFS (use .encrypted not .encryptedData)
        ...
        // ★ FIX Step 5: Save to IPFS-based file index with encryption metadata
  ...
  /**
   * Download and decrypt a file from the vault.
   * ★ FIX: Now actually decrypts if encryption metadata is available.
   */
  const downloadFile = useCallback(
    async (file: VaultFile): Promise<Uint8Array | null> => {
      ...
      try {
        ...
        // ★ FIX: Decrypt if file was encrypted and we have the key/iv
  ...
  /**
   * ★ NEW: Find a single file by CID (for FileDetailScreen navigation).
   */
