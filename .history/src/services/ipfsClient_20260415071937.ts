 ...
 * Accepts either Uint8Array or base64 string directly.
 */
async function uploadViaPinata(
  ...
  // Use pinJSONToIPFS endpoint — works reliably in React Native
  // FormData multipart upload is broken in React Native's fetch
  const url = `${PINATA_CONFIG.baseUrl}${PINATA_CONFIG.pinJSONEndpoint}`;
  const startTime = Date.now();

  // If data is already a base64 string, use it directly (avoids costly conversion)
 ...
 * Uses Pinata gateway first (since uploads go there), then falls back to others.
 *
...
export async function downloadFromIPFS(cid: string): Promise<ArrayBuffer> {
  ...
  // Try Pinata gateway first (content was uploaded there)
  const pinataGateway = 'https://gateway.pinata.cloud/ipfs/';
  try {
    const url = `${pinataGateway}${cid}?format=raw`;
    const controller = new AbortController();
    ...
    if (response.ok) {
      ...
      if (data.byteLength > 0) {
        const duration = Date.now() - startTime;
        logger.info(TAG, `Downloaded ${data.byteLength} bytes in ${duration}ms via Pinata gateway`);
        return data;
      }
    }
  } catch (error) {
    logger.warn(TAG, 'Pinata gateway download failed, trying fallback...', error);
  }

  // Fallback to other gateways
  const { data } = await fetchWithFallback(cid);
  const duration = Date.now() - startTime;
  logger.info(TAG, `Downloaded ${data.byteLength} bytes in ${duration}ms via fallback`);
...
export async function pinToIPFS(cid: string): Promise<PinResult> {
  ...
  try {
    // Use pinByHash to pin an existing CID that was uploaded elsewhere
