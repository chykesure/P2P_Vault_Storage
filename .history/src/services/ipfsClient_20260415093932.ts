...
export async function downloadFromIPFS(cid: string): Promise<ArrayBuffer> {
  ...
  // Unwrap pinJSONToIPFS wrapper to get the original raw bytes.
  // Supports two formats:
  //   NEW: { _v: 1, _d: "base64..." }  (current uploads)
  //   OLD: "base64..."                  (legacy uploads stored as raw JSON string)
  try {
    ...
    if (trimmed.startsWith('{')) {
      // NEW format: JSON object with _d field
      const parsed = JSON.parse(text);
      if (parsed && typeof parsed._d === 'string') {
        logger.info(TAG, `Unwrapping NEW format (from ${gatewayUrl})`);
      ...
      // OLD format: Pinata stored the base64 string as a JSON string
      const innerString = JSON.parse(text); // removes surrounding quotes
      if (typeof innerString === 'string' && innerString.length > 0) {
        logger.info(TAG, `Unwrapping OLD format (from ${gatewayUrl})`);
    ...
    // Not a wrapper — return raw bytes as-is
  } catch {
    // Not valid JSON — return raw bytes as-is
  }
