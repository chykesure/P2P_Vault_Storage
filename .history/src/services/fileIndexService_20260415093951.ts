...
async function loadIndex(address: string): Promise<FileIndexData> {
  ...
  if (cachedCID) {
    try {
      ...
      // Safety: ensure parsed data is an object with expected structure
      if (parsed && typeof parsed === 'object' && parsed.owner) {
        const index = parsed as FileIndexData;
        ...
        logger.warn(TAG, `Downloaded index is not a valid FileIndexData (got ${typeof parsed})`);
      }
    } catch (error) {
