  ...
  return (
    <Tab.Navigator
      screenOptions={({ route }) => ({
        ...
        tabBarStyle: {
          // Responsive: account for device bottom inset (Android nav bar, iOS home indicator)
          paddingBottom: Math.max(insets.bottom - 4, 5),
          paddingTop: 5,
          height: Platform.OS === 'android'
            ? 60 + Math.max(insets.bottom - 4, 0)
            : 60 + Math.max(insets.bottom - 4, 0),
          backgroundColor: '#fff',
...
export function AppNavigator() {
  ...
  // Determine which screen to show — but ONLY after SecureStore check completes
  ...
  // ⚠️ CRITICAL FIX: Wait for SecureStore check before deciding the initial route
  // Without this guard, isSetup/isUnlocked are both false (default) on cold start,
  // and the SetupPassword screen flashes every time.
  if (isChecking) {
