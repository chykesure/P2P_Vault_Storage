...
/**
 * Try to download from a CID by attempting gateways in order.
 * Tries ALL gateways regardless of health — health checks were causing
 * false negatives (e.g., Pinata auth endpoint needs JWT which we don't send).
 */
export async function fetchWithFallback(cid: string): Promise<{ data: ArrayBuffer; gatewayUrl: string }> {
  ...
  for (const gateway of IPFS_GATEWAYS) {
    try {
      ...
      console.warn(`[Gateway Fallback] Failed to fetch from ${gateway.name}:`, error);
      continue;
    }
  }
